...
在安装依赖之前，您需要配置必要的环境变量。

#### 5.2.1 前端配置

1.  在项目根目录（与 `package.json` 同级）下，创建一个名为 `.env.local` 的文件。
2.  打开 `.env.local` 文件，并添加您的通义千问API密钥，格式如下：
    ...
    **注意**:
    *   变量名必须以 `VITE_` 开头，这是Vite的安全要求。
    *   `.env.local` 文件已被添加到 `.gitignore` 中，不会被提交到版本库，请放心填写。

#### 5.2.2 后端配置

1.  进入 `backend` 文件夹。
2.  复制 `.env.example` 文件并将其重命名为 `.env`。
3.  打开新的 `.env` 文件，并填入您的语言模型API密钥和其他可能需要的配置。
