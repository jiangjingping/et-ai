...
npm run dev
```
该命令会启动一个本地开发服务器 (默认运行在 `http://localhost:3889`)。
...
加载WPS Office插件通常有以下几种方式：

1.  **开发时调试 (推荐):**
    *   WPS Office 提供了加载本地开发服务器URL作为插件的方式。
    *   打开WPS表格 -> "开发工具" -> "JS加载项" -> "管理加载项"。
    *   点击 "添加" 或 "新建"，在弹出的对话框中，选择 "从URL添加"，并输入开发服务器的地址 (例如 `http://localhost:3889/manifest.xml` 或直接 `http://localhost:3889`，具体取决于WPS版本和插件配置)。
    *   勾选启用该加载项。
2.  **加载构建后的本地插件:**
    *   将 `npm run build` 生成的 `dist` 目录下的所有文件，连同项目根目录的 `manifest.xml` 和 `public/ribbon.xml` (如果构建脚本没有自动处理它们到dist目录) 打包成一个 `.zip` 文件。
    *   将 `.zip` 文件后缀修改为 `.wpsaddon`。
    *   在WPS表格中，通过 "开发工具" -> "JS加载项" -> "管理加载项" -> "从本地文件添加"，选择该 `.wpsaddon` 文件。
    *   或者，将 `dist` 目录内容放置在WPS信任的本地目录，然后通过 "从文件夹添加" 的方式加载。
