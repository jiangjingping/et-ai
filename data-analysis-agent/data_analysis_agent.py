...
class DataAnalysisAgent:
    ...
    async def analyze(self, user_input: str, files: List[str] = None) -> Dict[str, Any]:
        """
        开始分析流程
        ...
        while self.current_round < self.max_rounds:
            ...
            try:                # 获取当前执行环境的变量信息
                ...
                messages = [
                    ...
                    {"role": "user", "content": self._build_conversation_prompt()}
                ]
                
                response = await self.llm.client.chat_completions_create(
                    messages=messages,
    ...
    async def _generate_final_report(self) -> Dict[str, Any]:
        ...
        try:            # 调用LLM生成最终报告
            # 构建包含 system prompt 的消息列表
            messages = [
                {"role": "system", "content": "你将会接收到一个数据分析任务的最终报告请求，请根据提供的分析结果和图片信息生成完整的分析报告。"},
            ...
            response = await self.llm.client.chat_completions_create(
                ...
                model=self.config.model,
                temperature=self.config.temperature,
                max_tokens=self.config.max_tokens # 使用配置中的max_tokens
            )
            response = response.choices[0].message.content
