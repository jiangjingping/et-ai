...
function OnWorkbookOpen(doc) {
    ...
    //非本地文件
    if (wps.PluginStorage.getItem("IsInCurrOADocOpen") == true) {
        allOpenDocFlagInfo[doc.DocID] = MyFlagInfo;
        if (allOpenDocFlagInfo[doc.DocID].editFlag) {
            let info = ''
            if (allOpenDocFlagInfo[doc.DocID].hasOwnProperty('isDraft')) {
                info = {
                    type: 'editDraft' // 起草来自本地的文档
                }
                allOpenDocFlagInfo[doc.DocID].btnFlags = [1,0]
            } else {
                info = 
                {
                    type: 'editDoc'
                }
                allOpenDocFlagInfo[doc.DocID].btnFlags = [1,1]
            }
            wps.OAAssist.WebNotify(JSON.stringify(info).replace(/\"/g,"'"), true);
        }
        if (allOpenDocFlagInfo[doc.DocID].printFile) {
            let info = 
            {
                type: 'print'
            }
            allOpenDocFlagInfo[doc.DocID].btnFlags = [0,1]
            wps.OAAssist.WebNotify(JSON.stringify(info).replace(/\"/g,"'"), true);
        }
        if (allOpenDocFlagInfo[doc.DocID].readFileFlag) {
            var info = 
            {
                type: 'opendoc'
            }
        ...
        OnWindowActivate()
        wps.PluginStorage.setItem("IsInCurrOADocOpen", false); // OA文档标志位恢复默认值
    }
...
function OnWorkbookNew(doc) {
    console.log('进入新建文档事件onDocumentNew')
    if (wps.PluginStorage.getItem("IsInCurrOADocOpen") == false) {
        // 如果是用户自己在WPS环境打开文档，则设置非OA文档标识
        console.log('用户自己在WPS环境新建文档，设置非OA文档标识')
        pSetNoneOADocFlag(doc);
    }
    //设置当前新增文档是否来自OA的文档
    // if (wps.PluginStorage.getItem("IsInCurrOADocOpen") == false) {
