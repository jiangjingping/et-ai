...
function OnUploadToServerSuccess(resp) {
    ...
    if (res.code != 1) {
        if (res.code == 4005) {
            ...
            allOpenDocFlagInfo[doc.DocID].btnFlags[0] = 0; //置灰OA助手上传
            wps.ribbonUI.InvalidateControl("btnSaveToServer"); //刷新OA助手上传按键状态
            //wps.ribbonUI.Invalidate();
        }
        wps.confirm(res.message)
        return
    }
    const attId = res.data[0].attId;
    const fileType = res.data[0].fileType;
    let jsonInfo = {
        Mode: "opendoc",
        Status: 1,
        Explain: attId,
    ...
    // SaveStatus = "uploadSuccess";
    wps.OAAssist.WebNotify(JSON.stringify(jsonInfo).replace(/\"/g,"'"), true);
    // wps.OAAssist.WebNotify(resp.replace(/\"/g,"'"));
    var l_doc = wps.EtApplication().ActiveWorkbook;
    // allOpenDocInfo[l_doc.DocID] = res.data[0].attId;
    var l_showConfirm = wps.PluginStorage.getItem("Save2OAShowConfirm");
    if (l_showConfirm) {
        openSave=true;
        console.log("openSave成功关闭")
        wps.alert("文件成功上传到服务器!")
        if (l_doc) {
            l_doc.Close(-1); //保存文档后关闭
            // wps.WpsApplication().Visible =false;
...
function OnBtnSaveToServer() {
    ...
    console.log('allOpenDocInfo', allOpenDocInfo)
    if (allOpenDocInfo[l_doc.DocID]) {
        const attIdIndex = l_uploadPath.indexOf('attId=')
        console.log('attIdIndex', attIdIndex)
        var urlPart
        if (attIdIndex == -1) {
            urlPart = l_uploadPath.slice(0, attIdIndex) + '&';
        } else {
            urlPart = l_uploadPath.slice(0, attIdIndex);
        }
        console.log('urlPart', urlPart)
        const urlAll = urlPart + "attId=" + allOpenDocInfo[l_doc.DocID]
        l_uploadPath = urlAll.replace('uploadAttachment', 'uploadLaterDelete'); // 文件上载路径 
        console.log('后端返回的attid', l_uploadPath)
    }
    ...
    // }
    var l_UploadName;
    if(Myinfo.newDocFilename)
    {
        l_UploadName = Myinfo.newDocFilename+".et";
    }else{
...
function OnBtnSaveAsLocalFile() {
    ...
    l_ksoFileDialog.Title='另存文件'
    
    if (l_ksoFileDialog.Show() == -1) { // -1 代表确认按钮
        l_doc.Unprotect(doc_ProtectPassword);
        //遍历设置每一张表格的编辑权限保护。
        for(i = 1; i <= l_doc.Worksheets.Count; i++) {
            l_doc.Worksheets.Item(i).Unprotect(doc_ProtectPassword);
        }
        l_doc.isOA = false;
...
function OnGetEnabled(control) {
    ...
    if (!doc || typeof(allOpenDocFlagInfo[doc.DocID]) === 'undefined' || typeof(allOpenDocFlagInfo[doc.DocID].btnFlags) === 'undefined') {
        btnFlags = [1,1];
    }
    else {
        btnFlags = allOpenDocFlagInfo[doc.DocID].btnFlags;
    }
    switch (eleId) {
        case "btnSaveToServer": //保存到OA服务器的相关按钮。判断，如果非OA文件，禁止点击
            return btnFlags[0] === 1 ? true : false;
        case "btnSaveAsFile":
            // let doc=wps.EtApplication().ActiveWorkbook;
            ...
            return true;
        case "FileSaveAsMenu":
            return pCheckIfOADoc( wps.EtApplication().ActiveWorkbook) === false; //OA文档下禁用另存为菜单
        default:
            ;
    }
    return true;
