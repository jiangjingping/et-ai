    ...
    if (l_strFileUrl) {
        ...
        if (pCheckIsExistOpenOADoc(l_strFileUrl) == true) {
            ...
            if (l_strFileUrl.startWith("http")) { // 网络文档
                DownloadFile(l_strFileUrl, params,function (path) {
                    if (path == "") {
...
function pOpenFile(doc, params, isOnlineDoc) {
    ...
    console.log(Myinfo.templateUrl)
    if (Myinfo.openDoc) {
        var MyMessage = {
            type:"openDoc"
        };
        wps.OAAssist.WebNotify(JSON.stringify(MyMessage).replace(/\"/g,"'"), true);
    }
    else if(params['extInfo'].jsonTemplateInfo)
    {   
        var jsonStrTemplateInfo = decodeURIComponent(escape(atob(params['extInfo'].jsonTemplateInfo)));
        GetBrowserTemplateData(wps.EtApplication().ActiveWorkbook, jsonStrTemplateInfo);
        wps.wps.EtApplication().Selection.FormFields.Shaded = false // 去除打印稿纸中的灰色区域
        const protectType = params['openType'].protectType;
        const ProtectPassword = params['openType'].password;
        doc.Protect(ProtectPassword);
    }
    //重新设置工具条按钮的显示状态
    pDoResetRibbonGroups();
    // 触发切换窗口事件
    // console.log("OnWindowActivate:" + Date());
    OnWindowActivate();
    // 新建文档时，给allOpenDocInfo中的attId赋初值
    allOpenDocInfo[doc.DocID] = "";
    return doc;
}
...
//     return l_Doc;
// }
var doc_ProtectPassword = ""
function pDoOpenOADocProcess(params, TempLocalFile) {
    console.log('pDoOpenOADocProcess：params', params)
    console.log('pDoOpenOADocProcess：编辑标志', params.extInfo.editFlag)
    ...
    for (var key = "" in params) {
        switch (key.toUpperCase()) //
        {
            ...
            case "openType".toUpperCase():
                l_ProtectType = params[key].protectType; //获取OA传来的文档保护类型
                l_ProtectPassword = params[key].password; //获取OA传来的保护模式下的文档密码
                doc_ProtectPassword = l_ProtectPassword;
                break;
    ...
    // if (params.extInfo.editFlag && params.extInfo.leaveMarkEdit || params.extInfo.readFileFlag) {
    //     l_bOpenRevision = true; //初始化开启修订模式
    //     l_bShowRevision = true; //初始化显示修订气泡样式
    // }
    var l_Doc;
    ifOpenReviewDoc = 1;
    l_Doc = wps.EtApplication().Workbooks.Open(TempLocalFile, false, false, undefined, l_strDocPassword);
...
function SetOADocProtect(doc, protectType, protectPassword) {
    ...
    if ([0, 1, 2, 3].indexOf(protectType) !== -1) {
        ...
        doc.Protect(protectPassword);
        //遍历设置每一张表格的编辑权限保护。
        for(i = 1; i <= doc.Worksheets.Count; i++) {
