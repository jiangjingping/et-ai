...
'- **禁止使用不存在的函数**: 严禁使用速查表中未列出的函数。例如，`dfd.to_datetime` 函数不存在，不要使用它。\n' +
...
'- **绘图由主线程处理**: 要创建图表，你必须使用 `generate_chart_from_code` 动作。此动作需要你编写一段JS代码，该代码的返回结果必须是一个完整的、符合ECharts格式的图表 `option` 对象。\n' +
        ...
        '  - `full_data`: **必须是**经过 `dfd.toJSON()` 转换后的**完整**DataFrame。这是为了在不同步骤间传递完整的数据状态。\n' +
        '  - `summary`: **必须是**一个**小型**的JSON对象，用于在反馈中显示。通常应为 `dfd.toJSON(df.head())` 或其他聚合结果。\n\n' +
        '---\n\n' +
...
'| 函数 | 描述 |\n' +
'|---|---|\n' +
'| `dfd.merge({left: df1, right: df2, on: [...]})` | 合并两个DataFrame。 |\n' +
'| `dfd.concat({dfList: [df1, df2], axis: 0/1})` | 连接两个DataFrame。 |\n\n' +
'---\n\n' +
...
'\n' +
'// 3. 转换为原生JS对象数组\n' +
'const rawData = dfd.toJSON(renamedDf);\n' +
'const longData = [];\n' +
'\n' +
'// 4. 遍历每一行，为每个类别创建新行\n' +
'rawData.forEach(row => {\n' +
