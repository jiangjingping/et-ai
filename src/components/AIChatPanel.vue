...
export default {
  ...
  setup() {
    ...
    const sendMessage = async () => {
      ...
      const onComplete = (finalContent) => {
        if (messages.value[aiMessageIndex]) {
          ...
          if (extractedChartOptions && extractedChartOptions.length > 0) {
            ...
            console.log('[AIChatPanel] 图表配置已提取:', extractedChartOptions);
          }
        }
      ...
      try {
        if (isTableContextAttached.value) {
          const rawData = utilFunctions.getTableContextDataAsRaw();
          if (rawData && rawData.length > 0) {
            await aiService.handleNormalPrompt(userMessageContent, rawData, onChunk, onComplete, onError);
          } else {
            addSystemMessage('⚠️ 未能获取到有效的表格数据。将按普通对话处理。');
            await aiService.callQwenAPIStream(userMessageContent, '你是一个友好、专业的AI助手。', onChunk, onComplete, onError);
          }
        } else {
          await aiService.callQwenAPIStream(userMessageContent, '你是一个友好、专业的AI助手。', onChunk, onComplete, onError);
        }
      } catch (error) {
        // onError回调已经处理了大部分错误，这里捕获未被处理的同步错误
        onError(error);
      }
    }
